...
/** Mounts the active POS theme on :root and re-applies when branding changes. */
export function useApplyPosTheme(branding: Branding | undefined) {
  useEffect(() => {
    if (!branding) return;
    const theme = findTheme(branding.themes, branding.posTheme) ?? branding.themes[0];
    if (theme) applyThemeTokens(theme.tokens);
    // Also update document title to brand name
    if (branding.name) document.title = `${branding.name} POS`;
  }, [branding]);
}
