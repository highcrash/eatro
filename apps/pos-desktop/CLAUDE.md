...
- **codecanyon branch** (1.0.x, license-client): publishes to channel
  ...
  were also built from this branch.
...
1. Bump `version` in `apps/pos-desktop/package.json` (codecanyon
   stays in 1.0.x; main stays in 2.0.x).
2. Append a section to `CHANGELOG.md`.
3. `git tag pos-desktop-v{version} && git push --tags`.
4. GitHub Actions builds `YourRestaurantPOS-Setup-{version}.exe` +
...
clean version split (1.x vs 2.x) is also a useful eyeball signal in
GitHub Releases.
