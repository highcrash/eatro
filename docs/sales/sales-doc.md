# Your Restaurant POS â€” The Restaurant Operating System
...
> 12-minute read Â· For owners and managers of dine-in restaurants,
> cafÃ©s, cloud kitchens, and small chains in Bangladesh and South Asia.
...
2. [What's different about Your Restaurant POS â€” the seven signals](#2-whats-different-about-restora-pos--the-seven-signals)
3. [The full stack â€” what's in the box](#3-the-full-stack--whats-in-the-box)
4. [Feature deep-dive â€” ten modules](#4-feature-deep-dive--ten-modules)
...
8. [Day 1 â†’ Month 1 â€” what go-live looks like](#8-day-1--month-1--what-go-live-looks-like)
...
mirrors â€” and pay the fines when the audit finds a gap.

**The internet drops in the middle of dinner service.** Your cloud-only
POS says "connectingâ€¦" and your cashier shrugs at the customer holding
...
POS quotes USD 79â€“299 per month per location. Three terminals across
two branches, twelve months a year â€” five years and you've paid more
...
chain â€” your software needs to match it.

**Your Restaurant POS is built for that exact stack of problems.** It's not a
generic POS we localised â€” it's a POS designed from day one around
...
## 2. What's different about Your Restaurant POS â€” the seven signals
...
generation is atomic per (branch, fiscal year, document kind) â€” a
single SQL `INSERT â€¦ ON CONFLICT DO UPDATE â€¦ RETURNING` so concurrent
...
- When connectivity returns, the outbox drains in FIFO order. Synthetic
  ...
  exact same response back from the server's idempotency cache â€”
  no double-charge, no double-Mushak.

The cashier doesn't see "syncingâ€¦", the customer doesn't see "card
...
- **Recipes** with cost roll-up. Every menu item costs are computed
  from the actual ingredient costs at the time of receipt â€” not from
...
- **Stock Reconciliation**. Print the count sheet, walk the storeroom,
  ...
  WASTE (with a reason â€” spoilage, prep error, contamination) or
...
post â€” discount name, value, menu item image â€” and queue it to your
...
## 3. The full stack â€” what's in the box
...
## 4. Feature deep-dive â€” ten modules
...
ESC/POS commands â€” no print dialog, no driver fight.
...
is always instant â€” the receipt prints, the drawer kicks, the customer
...
### 4.2 Kitchen â€” KDS + Section-Routed Thermal Tickets
...
an order is placed, the server fans out one ticket per station â€” to
...
with a date range â€” every PURCHASE, SALE, WASTE, ADJUSTMENT row
...
back in, and submit â€” every variance becomes a WASTE row (with a
...
â€” where did the rest go?" The Stock Watcher report tells you exactly:
...
spreadsheet â€” and a spreadsheet doesn't decrement on every sale.

---

### 4.4 Procurement â€” Suppliers, Purchasing, Shopping List, WhatsApp PO
...
afterthought â€” a separate page nobody uses. Your Restaurant POS makes it the
...
### 4.5 Bangladesh Compliance â€” Mushak 6.3 / 6.8
...
call runs inside the same transaction â€” generates the next sequential
...
POS sometimes have Mushak but rarely with atomic serial generation â€”
...
your branch's WiFi (so customers can't order from outside) â€” useful
...
hire a separate "online ordering platform" â€” same database, same
...
doesn't really do it. None of them IP-gate to a branch â€” they assume
you want delivery aggregator orders too.

---

### 4.7 Customers & Marketing â€” DB, Discounts, Coupons, Auto-Facebook, SMS
...
a Facebook post â€” discount name, value, menu item image, copy â€” and
...
**Why it matters to you as the owner.** "Mid-week 20% off Biryani" â€”
...
### 4.8 People â€” Staff, Custom Roles, Attendance, Payroll, Leave
...
Cashier Permissions matrix lives on top â€” for example, even a CASHIER
...
can apply a 50% discount?" "Who can issue a refund?" â€” every one of
...
### 4.9 Finance â€” Expenses, Liabilities, Accounts, Reports
...
underlying transactions â€” no overnight ETL, no stale dashboard.
...
### 4.10 Audit & Trust â€” Activity Log, Void Audit, Permissions
...
Activity Log page renders the diff field-by-field â€” recipe edits show
"Spring Onion: 20g â†’ 50g + Garlic added", menu edits show
"Cost Per Unit: 12.50 â†’ 15.00", with cuids and Decimals
...
returned to stock (no kitchen loss â€” cashier mistake) and those logged
as waste (food gone â€” kitchen loss). For each, the selling-price
total. For the wasted bucket, the actual ingredient-cost loss.

**Why it matters to you as the owner.** "Why is my food cost
percentage suddenly higher this week?" â€” the Void Audit shows you
that someone's voiding-as-waste 12 times a day. "Who changed the
discount cap last Tuesday?" â€” Activity Log, takes 5 seconds. Audit
...
- Every cell where Your Restaurant POS has âœ… and three+ competitors have âŒ is
...
- Toast is the closest in feature depth â€” but it's US-first,
  hardware-locked, and SaaS-priced.
- Petpooja is the closest in regional fit â€” but it's India-VAT-first,
  cloud-only, and SaaS-priced.
- Loyverse / Square are entry-level â€” fine for a coffee cart, not for
  a real restaurant.
...
We're not going to quote a price in this document â€” pricing is best
discussed once we know your branch count and terminal count. But here's
the framing.

**A typical mid-market POS subscription is USD 79â€“299 per month, per
location.** Three terminals across two branches, twelve months a year,
five years: **USD 5,000â€“18,000 in subscription fees alone**, before
hardware, before payment processing fees, before add-ons. And at the
end of five years you own nothing â€” stop paying, and the software
locks.

**Your Restaurant POS is a one-time licence.** You pay once. You get every
module listed in Â§3. You can run it on a single VPS that costs a few
thousand taka per month. You can run it on hardware you already own â€”
...
|---|---|---|
| Software, 5 years | USD 5,000â€“18,000 | One-time licence |
...
| Hosting | Their cloud (you pay forever) | Your VPS (a few thousand à§³/mo) |

The total-cost-of-ownership delta over five years is large enough that
most owners pay back the licence in the first quarter alone â€” and own
...
- Independent dine-in restaurants, **5â€“80 covers**.
- CafÃ©s, quick-service, cloud kitchens.
- Small chains, **2â€“10 branches**.
...
- 50+ branch enterprise chains â€” no franchise tooling, no
  centralised purchasing across hundreds of locations.
- Food trucks â€” no GPS routing or moving-location handling.
- Grocery / retail â€” no barcode scanning, no SKU-driven inventory.
- Owners who want a SaaS dashboard they never have to think about â€”
  Your Restaurant POS is more powerful, but you do need to set up your own
  VPS (or have a partner do it for you).

---

## 8. Day 1 â†’ Month 1 â€” what go-live looks like
...
| When | What | Time |
|---|---|---|
| **Day 0** | Install on your VPS via the Install Wizard (Postgres + Node) | 1â€“2 hours |
| **Day 1** | Pair the first terminal, load menu, configure printers | 3 hours |
| **Day 2** | Train cashiers on PIN login + order entry | 2 hours |
| **Week 1** | Go live â€” Mushak invoices auto-issuing on every payment | â€” |
| **Week 2** | Configure kitchen sections + recipes + supplier list | 4â€“6 hours |
| **Month 1** | Coupons + auto-Facebook + attendance + payroll + reports | 6â€“8 hours |
...
- **Idempotent server endpoints.** The desktop outbox can replay any
  mutation without double-writing â€” every endpoint that matters is
...
- **Daily automated database backups.** Plus on-demand backup
  download â€” you always have a copy.
